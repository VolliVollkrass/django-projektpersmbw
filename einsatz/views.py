...
def einsatz_update(request, pk):
    ...
    return render(request, "einsatz/einsatz_form.html", {
        "form": form,
        "einsatz": einsatz,
        "modus": "edit"
    })
...
def einsatz_delete(request, pk):
    ...
    if request.method == "POST":
        ...
        if confirm_text == "Löschen":
            einsatz.delete()
            messages.success(request, f'Einsatz*in erfolgreich gelöscht.')
            return redirect("einsatz")  # anpassen an deine URL
